...
static void emit_loop(int loop_start) {
    ...
    /* +2 to take into account the size of OP_LOOP's own operands,
...
static void class_declaration() {
    ...
    if (match(TOKEN_LESS)) {
        ...
        /* Create a new scope and make superclass a local variable */
        begin_scope();
...
static void return_statement() {
    if (current->type == TYPE_SCRIPT) {
        error("Can't return from top-leve code.");
    }
    if (match(TOKEN_SEMICOLON)) {
        emit_return();
    }
...
static void string(bool can_assign) {
    /* +1 and -2 trim he leading and trailing quotation marks */
